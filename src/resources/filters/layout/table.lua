...
function tablePanel(divEl, layout, caption, options)
  ...
  for i, row in ipairs(layout) do
    ...
    local aligns = row:map(function(cell) 
      ...
      if tableFromLayoutCell(cell) and _quarto.doc.isDocxOutput() and align == "center" then
...
function tableCellContent(cell, align, options)
  ...
  if image then
    ...
    if #image.caption > 0 and (_quarto.doc.isRtfOutput() or _quarto.doc.isOdtOutput()) then

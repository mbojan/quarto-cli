...
function columns() 
  ...
  return {
    ...
    Span = function(el)
      ...
      if _quarto.doc.isLatexOutput() and hasMarginColumn(el) then 
...
function renderDivColumn(el) 
    ...
    if captionEl and codeEl then
      ...
      if #columnClasses > 0 then
        ...
        for i, clz in ipairs(columnClasses) do 
          if clz == kSideCaptionClass and _quarto.doc.isHtmlOutput() then
  ...
  elseif _quarto.doc.isLatexOutput() and not requiresPanelLayout(el) then

...
function callout() 
  return {
    ...
    Blocks = function(blocks)
      if _quarto.doc.isDocxOutput() then
    ...
    Div = function(div)
      if div.attr.classes:find_if(isCallout) then
        ...
        if _quarto.doc.isHtmlOutput() and hasBootstrap() then
          return calloutDiv(div) 
        elseif _quarto.doc.isLatexOutput() then
          return calloutLatex(div)
        elseif _quarto.doc.isDocxOutput() then
          return calloutDocx(div)
        elseif _quarto.doc.isEpubOutput() or _quarto.doc.isRevealJsOutput() then
...
function calloutDocxDefault(div, type, hasIcon)
  ...
  if contents:find_if(function(el) 
    return el.t == "Div" and el.attr.classes:find_if(_quarto.doc.isDocxOutput) ~= nil 
  end) ~= nil then
